...
class NCCMetric(Metric):
    def __init__(self, win=9, dist_sync_on_step=False):
        super().__init__(dist_sync_on_step=dist_sync_on_step)
        # We track the sum of NCC and the number of samples to get the mean
        self.add_state("sum_ncc", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")
        self.win = win

    def update(self, y_pred, y_true):
        # Your math logic goes here
        Ii = y_true
        Ji = y_pred
        ...
        # Accumulate state
        self.sum_ncc += torch.mean(cc)
        self.total += 1

    def compute(self):
        return self.sum_ncc / self.total
...
class NCC_vxm(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        # Avoid negative values in variance due to numerical errors
        I_var = I_var.clamp(min=1e-8)
        J_var = J_var.clamp(min=1e-8)



        # print(f"local variances : {torch.mean(I_var), torch.mean(J_var)}")
        ...
        # print(f"cross_correlation value : {torch.mean(cc)}")


        # return -torch.mean(cc)
        return -m
    
class Global_NCC (nn.Module) : 
    def __init__(self):
        super(Global_NCC, self).__init__()

    def forward(self, x, y, return_map=False, reduction='mean', eps=1e-8):
